...
//
//    @Query("SELECT COALESCE(SUM(b.totalPrice), 0) FROM Booking b WHERE b.bookingDate BETWEEN :fromDate AND :toDate AND
// b.status = 'CONFIRMED'")
...
//            @Param("toDate") LocalDateTime toDate
//    );
//    @Query("SELECT b.id1.id,b.showID.id,bd.seatNumber FROM Booking b JOIN BookingDetail bd ON bd.bookingID = b WHERE
// b.status = 'CONFIRMED'")
...
public interface BookingRepository extends JpaRepository<Booking, Long> {
    @Query("SELECT b FROM Booking b WHERE "
            + "(:userName IS NULL OR LOWER(b.id1.userName) LIKE LOWER(CONCAT('%', :userName, '%'))) AND "
            + "(:showId IS NULL OR b.showID.id = :showId) AND "
            + "(:status IS NULL OR b.status = :status) AND "
            + "(:fromDate IS NULL OR b.bookingDate >= :fromDate) AND "
            + "(:toDate IS NULL OR b.bookingDate <= :toDate)")
    ...
    @Query(
            "SELECT COALESCE(SUM(b.totalPrice), 0) FROM Booking b WHERE b.bookingDate BETWEEN :fromDate AND :toDate AND b.status = 'CONFIRMED'")
    Long sumRevenueByDateRange(@Param("fromDate") LocalDateTime fromDate, @Param("toDate") LocalDateTime toDate);
    ...
    // Revenue by date for charts
    @Query("SELECT function('date', b.bookingDate) as d, COALESCE(SUM(b.totalPrice), 0) as revenue " + "FROM Booking b "
            + "WHERE b.bookingDate BETWEEN :fromDate AND :toDate AND b.status = 'CONFIRMED' "
            + "GROUP BY function('date', b.bookingDate) "
            + "ORDER BY function('date', b.bookingDate)")
    List<Object[]> getRevenueByDate(@Param("fromDate") LocalDateTime fromDate, @Param("toDate") LocalDateTime toDate);

    // Top movies by bookings
    @Query("SELECT m.title, COUNT(b.id) as bookingCount, COALESCE(SUM(b.totalPrice), 0) as revenue " + "FROM Booking b "
            + "JOIN b.showID s "
            + "JOIN s.movieID m "
            + "WHERE b.bookingDate BETWEEN :fromDate AND :toDate AND b.status = 'CONFIRMED' "
            + "GROUP BY m.id, m.title "
    ...
    @Query(
            "SELECT b.id1.id,b.showID.id,bd.seatNumber FROM Booking b JOIN BookingDetail bd ON bd.bookingID = b WHERE b.status = 'CONFIRMED'")
    List<Object[]> findAllByBookingConfirmed();
